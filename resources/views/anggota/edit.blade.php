    ...
    <form action="{{ route('anggota.update', $anggota->id) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="form-group">
        ...
        <select name="jenis_anggota_id" id="jenis_anggota_id" class="form-control" required>
          ...
          <option value="{{ $jenis->id }}" {{ $jenis->id == $anggota->jenis_anggota_id ? 'selected' : '' }}>
            {{ $jenis->jns_anggota }}
          </option>
          @endforeach
        </select>
      ...
      <div class="form-group">
        ...
        <textarea name="keterangan" id="keterangan" class="form-control">{{ old('keterangan', $anggota->keterangan) }}</textarea>
      </div>

      <div class="form-group">
        <label for="foto">Foto</label>
        @if($anggota->foto)
        <p><img src="{{ asset('storage/' . $anggota->foto) }}" alt="Foto Anggota" class="img-thumbnail" width="150"></p>
        @endif
